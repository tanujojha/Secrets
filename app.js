...
passport.use(new GoogleStrategy({ //using google Strategy and setting the options
  ...
  function(accessToken, refreshToken, profile, cb) {
    ...
    User.findOrCreate({
      googleId: profile.id
    }, function(err, user) { //finding or creating user with the googleId
      return cb(err, user);
    });
  }
));
...
app.get("/auth/google",                    //authenticating at the /auth/google route
...
app.post("/submit", function(req, res) {
  ...
  // console.log(req.user.id);            //loging the user thats logged in and clicked the submit button
  ...
  User.findById(req.user.id, function(err, foundUser) {     //finding the user that posted their secrete to add in db
    ...
    if (err) {
      ...
      User.findByIdAndUpdate(req.user.id, { $push: { secrets: submittedSecret }}, function(err) {   //pushing the value in the secrets array
...
app.get("/secrets", function(req, res) {
  ...
  User.find({ secrets: { $ne: null } }, function(err, foundUsers) {       //finding all the users with the field secrets not  equal ($ne) to null
    // console.log(foundUsers);       //logs array of users with matching conditions
    if (!err) {
      res.render("secrets", {
